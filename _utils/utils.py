...
def get_random_data(line: str,
    ...
    if not random:
        ...
        # correct boxes
        # 建立(max_boxes,5)的全零矩阵，用于存category_id,centter_point_x,
        # center_point_y,width,height
...
def preprocess_true_boxes(true_boxes: np.ndarray,
    ...
    true_boxes_xy = (true_boxes[..., [0, 1]] + true_boxes[..., [2, 3]]) // 2
    true_boxes_wh = true_boxes[..., [2, 3]] - true_boxes[..., [0, 1]]


    # 注意:这是的true_box里存的时中心点坐标和高宽，框中的数据压缩到[0,1]
    # 这样处理方便映射到gird中，便于将真实框调整到13*13，26*26，52*52之间
    true_boxes[..., [0, 1]] = true_boxes_xy / input_shape[::-1]
    true_boxes[..., [2, 3]] = true_boxes_wh / input_shape[::-1]
    # (x,y,w,h)
    ...
    # y_true 的格式为(m,13,13,3,c)(m,26,26,3,c)(m,52,52,3,c)  c=4+1+num_classes
